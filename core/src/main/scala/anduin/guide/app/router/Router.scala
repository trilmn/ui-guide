...
object Router {
  ...
  val config: RouterConfig[Page] = RouterConfigDsl[Page].buildConfig { dsl =>
    ...
    (trimSlashes
      ...
      | dynamicRouteCT("button-full" ~ hash.caseClass[Button]) ~> getRender(() => Promise.resolve[RenderFn](PageButtonFull.render _))
      | dynamicRouteCT("button-ghost" ~ hash.caseClass[Button]) ~> getRender(() => Promise.resolve[RenderFn](PageButtonGhost.render _))
      | dynamicRouteCT("button-minimal" ~ hash.caseClass[Button]) ~> getRender(() => Promise.resolve[RenderFn](PageButtonMinimal.render _))
      | dynamicRouteCT("button-text" ~ hash.caseClass[Button]) ~> getRender(() => Promise.resolve[RenderFn](PageButtonLink.render _))
